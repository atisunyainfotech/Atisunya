...
const features = [
  {
    ...
    desc: "Capture check-in, check-out, shift timing, late marks, overtime, and leave status with clean daily visibility.",
    icon: Clock3,
  },
  {
    title: "Location Intelligence",
    desc: "Support field teams with geo-tagged attendance, branch-wise views, and practical workforce movement checks.",
    icon: MapPinned,
  },
  {
    title: "Employee Identity",
    desc: "Keep attendance secure with user profiles, role-based access, device rules, and verified workforce records.",
    icon: Fingerprint,
  },
  {
    title: "Payroll Readiness",
    desc: "Turn attendance data into payroll-ready summaries, exception reports, and approval trails for HR and finance.",
    icon: CalendarClock,
  },
];

const outcomes = [
  "Lower attendance disputes",
  "Faster payroll preparation",
  "Cleaner branch-level reporting",
  "Better field workforce control",
];
        ...
        <div className="container grid min-h-[calc(100vh-82px)] gap-14 py-20 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          <motion.div initial={{ opacity: 0, y: 34 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <h1 className="max-w-4xl text-5xl font-black leading-tight md:text-7xl">
              Time Stamp for precise workforce intelligence.
            </h1>
            <p className="mt-7 max-w-2xl text-lg leading-8 text-white/76">
              A premium attendance and workforce visibility platform for teams
              that need accurate time capture, location clarity, payroll inputs,
          ...
          <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} className="rounded-[24px] border border-white/14 bg-white/10 p-5 shadow-[0_36px_100px_rgba(0,0,0,0.28)]">
            <div className="rounded-[18px] bg-white p-6 text-[#002050]">
              <div className="flex items-center justify-between border-b border-[#002050]/10 pb-5">
                <div>
                  <h2 className="text-2xl font-black">Attendance Command</h2>
                  <p className="mt-1 text-sm text-slate-500">Live workforce snapshot</p>
                </div>
                <ShieldCheck className="text-[#2F7F78]" size={34} />
              </div>
              ...
              <div className="mt-6 rounded-[14px] bg-[#F6F9FC] p-5">
                ...
                <div className="flex h-32 items-end gap-3">
                  {[66, 82, 74, 91, 88, 96].map((height) => (
                    <span key={height} className="flex-1 rounded-t-[8px] bg-[linear-gradient(180deg,#fbc02d,#2F7F78)]" style={{ height: `${height}%` }} />
                  ))}
                </div>
              </div>
            </div>
        ...
        <div className="container">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-4xl font-black leading-tight text-[#002050] md:text-5xl">
              Built for HR teams, branch managers, and field operations
            </h2>
          </div>
